...
def test_perfect(tree_factory: Callable[..., EvalNode]) -> None:
    root = tree_factory({"a": 1, "b": 2}, {"a": 1, "b": 2})
    assert OverallLeafScore().compute(root) == 1.0


def test_half(tree_factory: Callable[..., EvalNode]) -> None:
    root = tree_factory({"a": 1, "b": 9}, {"a": 1, "b": 2})
    assert OverallLeafScore().compute(root) == pytest.approx(0.5)


def test_weighted_by_field_weight(tree_factory: Callable[..., EvalNode]) -> None:
    cfg = EvalConfig(
...
def test_uses_each_leaf_key_metric(tree_factory: Callable[..., EvalNode]) -> None:
    cfg = EvalConfig(
...
def test_empty_tree_is_vacuously_one(tree_factory: Callable[..., EvalNode]) -> None:
    root = tree_factory({}, {})
    assert OverallLeafScore().compute(root) == 1.0
