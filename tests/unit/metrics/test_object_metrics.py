...
def test_f1_perfect(tree_factory) -> None:
    root = tree_factory({"a": 1, "b": 2}, {"a": 1, "b": 2})
    assert ObjectF1().compute(root) == 1.0


def test_f1_one_wrong(tree_factory) -> None:
...
def test_precision_recall_with_missing_and_extra(tree_factory) -> None:
...
def test_extra_keys_ignored_by_default(tree_factory) -> None:
...
def test_prf1_returns_dict(tree_factory) -> None:
    root = tree_factory({"a": 1, "b": 9}, {"a": 1, "b": 2})
...
def test_soft_mode_fractional(tree_factory) -> None:
    ...
    soft = ObjectF1(score_policy={"name": "token_f1"}, mode=GradingMode.SOFT).compute(root)
    assert 0.0 < soft < 1.0


def test_validity_type_check(tree_factory) -> None:
...
def test_validity_checks_container_types(tree_factory) -> None:
...
def test_validity_vacuous_when_no_scalars(tree_factory) -> None:
    root = tree_factory({}, {})
    assert ObjectTypeValidity().compute(root) == 1.0


def test_empty_object_vacuously_perfect(tree_factory) -> None:
    root = tree_factory({}, {})
    assert ObjectF1().compute(root) == 1.0
    assert ObjectAccuracy().compute(root) == 1.0
...
def test_f1_weighted_by_default(tree_factory) -> None:
...
def test_f1_weight_mode_none_restores_plain(tree_factory) -> None:
    root = _weighted_tree(tree_factory)
    assert ObjectF1(weight_mode=WeightMode.NONE).compute(root) == pytest.approx(0.5)


def test_accuracy_weighted_by_default(tree_factory) -> None:
    root = _weighted_tree(tree_factory)
    # numer = 1*1 + 3*0 = 1, denom = 1+3 = 4 → 0.25 (vs 0.5 unweighted)
    assert ObjectAccuracy().compute(root) == pytest.approx(0.25)
    assert ObjectAccuracy(weight_mode=WeightMode.NONE).compute(root) == pytest.approx(0.5)


def test_weighted_missing_field(tree_factory) -> None:
    from structured_eval import FieldConfig
