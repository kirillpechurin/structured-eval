...
| Class             | Key                 | Measures                                                        |
|-------------------|---------------------|----------------------------------------------------------------|
| [`ExactMatch`](exact-match.md)         | `exact_match`       | exact equality (1.0 / 0.0); the default scalar metric          |
| [`NormalizedMatch`](normalized-match.md) | `normalized_match`  | equality after trimming/lowercasing — ignores casing & spacing |
| [`Numeric`](numeric.md)             | `numeric`           | numeric equality within a tolerance (strips currency/separators)|
| [`NumericCloseness`](numeric-closeness.md) | `numeric_closeness` | graded numeric closeness `1 − |a−e| / max(|a|,|e|)`            |
| [`TokenF1`](token-f1.md)             | `token_f1`          | token-overlap F1 — rewards partial phrase matches              |
| [`Fuzzy`](fuzzy.md)               | `fuzzy`             | fuzzy string similarity (rapidfuzz; configurable method)       |
| [`Levenshtein`](levenshtein.md)         | `levenshtein`       | edit-distance ratio (thin alias of `Fuzzy(RATIO)`)             |
| [`Presence`](presence.md)            | `presence`          | whether the field is present / non-null                        |
| [`TypeMatch`](type-match.md)           | `type_match`        | whether actual and expected share a JSON type                  |
...
| Class             | Key                  | Measures                                            |
|-------------------|----------------------|-----------------------------------------------------|
| [`ObjectAccuracy`](object-accuracy.md)    | `object_accuracy`    | fraction of children that pass; the default object metric |
| [`ObjectPrecision`](object-precision.md)   | `object_precision`   | of the keys present, how many are right             |
| [`ObjectRecall`](object-recall.md)      | `object_recall`      | of the expected keys, how many are right            |
| [`ObjectF1`](object-f1.md)          | `object_f1`          | harmonic mean of object precision & recall          |
| [`ObjectPRF1`](object-prf1.md)        | `object_prf1`        | precision + recall + F1 together (one metric, three keys) |
| [`ObjectTypeValidity`](object-type-validity.md) | `object_type_validity` | fraction of children with the expected JSON type (count-based) |
...
| Class             | Key                | Measures                                          |
|-------------------|--------------------|---------------------------------------------------|
| [`ArrayAccuracy`](array-accuracy.md)     | `array_accuracy`    | fraction of elements correctly matched; the default array metric |
| [`ArrayPrecision`](array-precision.md)    | `array_precision`   | of the produced elements, how many matched        |
| [`ArrayRecall`](array-recall.md)       | `array_recall`      | of the expected elements, how many were found     |
| [`ArrayF1`](array-f1.md)           | `array_f1`          | harmonic mean of array precision & recall         |
| [`ArrayPRF1`](array-prf1.md)         | `array_prf1`        | precision + recall + F1 together                  |
| [`ArrayCardinality`](array-cardinality.md)   | `array_cardinality` | how close the element count is, ignoring matching |
...
|-------------------|----------------------|--------------|---------------------------------------------------|
| [`OverallLeafScore`](overall-leaf-score.md)   | `overall_leaf_score`  | `expected`   | weighted mean over every scalar leaf of the tree  |
| [`CoverageLeafScore`](coverage-leaf-score.md)  | `coverage_leaf_score` | `expected`   | fraction of expected leaves that are present      |
| [`SchemaValidity`](schema-validity.md)     | `schema_validity`     | a schema     | validity against a JSON Schema / pydantic model   |
| [`RulePassRate`](rule-pass-rate.md)       | `rule_pass_rate`      | a rule set   | fraction of business rules that hold              |
| [`Faithfulness`](faithfulness.md)       | `faithfulness`        | `source`     | whether values are grounded in the source text    |

## Representative metric

| Class             | Key          | Branch     | Measures                                  |
|-------------------|--------------|------------|-------------------------------------------|
| [`MeanScore`](mean-score.md)         | `mean_score` | any-node   | mean of a node's *own* metrics (its default `key_metric`) |
...
- [`ExactMatch`](exact-match.md) — the default scalar metric, a good first page.
